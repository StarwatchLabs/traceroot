...
/**
 * Whether detection is queued for a trace, and which detectors to expect.
 *
 * Powers the trace page's live detector updates: the pipeline is debounced by
 * ~a minute, so this is what lets the UI say "detection in progress" right away
 * and stop polling the instant it knows nothing is coming (`sampled_out`) or
 * every expected run has landed.
 *
 * Degrades to an empty state on any backend failure — this is a freshness hint,
 * not data the page needs, so a client should fall back rather than error out.
 */

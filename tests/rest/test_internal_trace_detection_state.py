...
class TestTraceDetectionState:
    ...
    def test_reports_pending_with_expected_detectors(self, client, mock_redis, secret):
        # The expected detector ids are what let the client stop polling exactly
        # when every enqueued run has landed.
    ...
    def test_reports_sampled_out(self, client, mock_redis, secret):
        # Sticky "nothing will ever appear" — the client must not poll at all.
    ...
    def test_reads_the_worker_claim_key(self, client, mock_redis, secret):
        ...
        # Must match worker.detector_tasks._lock_key or the state is never found.
    ...
    def test_missing_record_is_no_signal(self, client, mock_redis, secret):
        # Expired past TTL / never detected: absence is NOT a promise of nothing,
        # so the client falls back to its own window rather than giving up.
